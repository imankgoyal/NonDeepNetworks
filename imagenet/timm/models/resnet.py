...
from .simplenet_impl.utils import SE1 as SE_Our
...
default_cfgs = {
    ...
    'resnet14': _cfg(url=''),
    'resnet14_wide': _cfg(url=''),
    'resnet12_wide': _cfg(url=''),
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
        ...
        # Feature Blocks
        if our_ver:
            channels = [base_width * x for x in [1, 2, 4, 8]]
        else:
...
def resnet14(pretrained=False, **kwargs):
    ...
    return _create_resnet('resnet14', pretrained, **model_args)


@register_model
def resnet14_wide(pretrained=False, **kwargs):
    """Constructs a ResNet-14 model.
    """
    model_args = dict(block=Bottleneck, layers=[1, 1, 1, 1], base_width=64 * 3, **kwargs)
    return _create_resnet('resnet14_wide', pretrained, **model_args)


@register_model
def resnet12_wide(pretrained=False, **kwargs):
    model_args = dict(block=BasicBlock, layers=[1, 1, 1, 2], base_width=64 * 2,
                      our_ver=True, **kwargs)
    return _create_resnet('resnet12_wide', pretrained, **model_args)

@register_model
def resnet12_wide_swish_se(pretrained=False, **kwargs):
    model_args = dict(block=BasicBlock, layers=[1, 1, 1, 2],
                      base_width=int(64 * 1.95),
                      our_ver=True, act_layer=nn.SiLU,
                      block_args=dict(attn_layer='our_se'), **kwargs)
    return _create_resnet('resnet12_wide', pretrained, **model_args)
