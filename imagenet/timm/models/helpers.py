...
def load_checkpoint(model, checkpoint_path, use_ema=False, strict=True):
    ...
    if not strict:
        print("Removing head.fc.weight and head.fc.bias")
        del state_dict['head.fc.weight']
        del state_dict['head.fc.bias']
        print("Initializing head.fc.weight and head.fc.bias to zero")
        nn.init.zeros_(model.head.fc.weight)
        nn.init.zeros_(model.head.fc.bias)
    model.load_state_dict(state_dict, strict=strict)

def update_class_head(model, num_classes):
    device = next(model.parameters()).device
    model.head = ClassifierHead(
        **{**model.head.args, 'num_classes': num_classes}
    ).to(device)
    nn.init.zeros_(model.head.fc.weight)
    nn.init.zeros_(model.head.fc.bias)
